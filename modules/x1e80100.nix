...
let
  ...
  cfg = config.x1e.device;
in
{
  options.x1e.device = lib.mapAttrs (_: device: {
    enable = lib.mkEnableOption "support for the ${device.displayName}";
  }) devices;

  config = lib.mkMerge [
    # Set the default device tree based on x1e.device.*.enable
    (lib.mkMerge (
      ...
      assertions = [
        {
          ...
          message = ''
            ...
            ${lib.concatStringsSep "\n" (lib.mapAttrsToList (key: _: "x1e.device.${key}.enable") devices)}
          '';
        }
      ];
