...
If you would like to attempt using this on something other than the Lenovo Yoga Slim 7x, enable the appropriate `x1e.device.*.enable` option.
...
and then use the `x1e-nixos-config.nixosModules.x1e` module. Set one of the `x1e.device.*.enable` options.
